  ...
  function logout() {
	...
	if ( !isset($_SESSION['USER_NAME']) )
	{
		...
		else
		{
			...
			{
				$login_username = trim(strtolower($login_username));
				...
				ldap_bind($connLdap) or $skipLdap = true;
				if (!$skipLdap)
				{
					$result = ldap_search($connLdap,"ou=People,dc=ldap,dc=internal,dc=quantumsignal,dc=com","uid=".$login_username);
					$info = ldap_get_entries($connLdap, $result);
					if (isset($info[0]['dn']))
					{
						$inLdap = true;
						if (ldap_bind($connLdap, $info[0]['dn'], $login_passwordHash)) $ldapPass = true;
					}
				}
				...
				elseif($ldapPass)
				{
					//$msg = "Username in LDAP but local Project Tracking account required!";
					$db = new DBManager();
			        $pdo = $db->pdo();
					
					$userName = $login_username;
					$thisuser = new LdapUser($userName);
					$fullName = $thisuser->getFullName();
					
					if (LdapGroup::isUserInGroup($userName,"qsadmin"))
						$role = "ADMINISTRATOR";
					else
						$role = "USER";
			        ...
			        $sth->execute( array ('username' => $login_username, 'passwordHash' => md5($login_username.":timekeeping:".$login_passwordHash), 'fullName' => $fullName, 'state' => "active", 'parttime' => 0 ) );
			        $mysql_id = $pdo->lastInsertId();
					$sth = $pdo->prepare( "REPLACE INTO Roles ( UserID, Role, CreateDate, LastUpdate ) VALUES ( :UserID, :Role, NOW(), NOW() )" );
					$sth->execute( array( 'UserID' => $mysql_id, 'Role' => $role ) );
				}
				else
				{								
					$msg = "Unknown user";
