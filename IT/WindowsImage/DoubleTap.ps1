...
if(!(Test-Path -Path C:\renamed.txt))
{
  ...
  {
    $calcName = ($hostname + $hostname).SubString(0,5)
  }
  $hostname = $hostname.ToLower()
  $calcName = $hostname.ToLower()
  $host5 = $calcName
...
# Windows info and DoubleTap timestamp
#
$Winfo = Get-ComputerInfo -Property "os*"
$time  = Get-Date
$hostname = hostname
$infoString = "Windows Edition: " + $Winfo.OsName + "`r" + "Version: " + $Winfo.OsVersion + "`r" + "Image: April 2021 `r" + "Windows originally installed on: " + $Winfo.OsInstallDate + "`r" + "Host imaged on: " + $time + "`r"
$infoString | Out-File -FilePath C:\Windows\$hostname.txt
...
#Join the machine to the domain and put it in the Workstations OU with the provided credentials, then restart
Add-Computer -DomainName ad.internal.quantumsignal.com -OUPath "OU=Workstations,DC=ad,DC=internal,DC=quantumsignal,DC=com" -Credential $admin -Restart
